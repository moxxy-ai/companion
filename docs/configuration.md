...
| `COMPANION_RATE_LIMIT` | `240` | Unauthenticated `/api` requests allowed per minute per client address; `0` disables the budget. Refusals answer 429 with `Retry-After`. Authenticated traffic is deliberately not capped: an account is revocable, the SPA polls, and CLI/MCP clients burst. Behind a proxy this keys on the address resolved through `COMPANION_TRUSTED_PROXIES`, so set that first or every caller shares one bucket. |
